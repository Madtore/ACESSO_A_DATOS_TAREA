...
class Empleado():        
    def __init__(self, **kwargs):
        ...
        self.feche_baja = kwargs.get("feche_baja" or None)
        
        
    def __str__(self):
        return f"{self.apellido_nombre} - {self.salario_mensual} - {self.paga_extra} - {self.departamento} - {self.puesto} - {self.telefono} - {self.email} - {self.direccion} - {self.nif} - {self.datos_bancarios} - {self.numero_seguro_social} - {self.genero} - {self.fecha_inicio} - {self.fecha_nacimiento} - {self.irpf} - {self.seg_social}"  